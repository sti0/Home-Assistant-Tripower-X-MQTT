...
def updateTripower(cfg, mqtt_settings):
    ...
    while exitafter is None or exitafter > 0:
        ...
        try:
            ...
            for d in data:
                dname = cfg.get("SensorPrefix", "") + d["channelId"].replace(
                    "Measurement.", ""
                ).replace("[]", "")
...
def updatePowerMeter(cfg, mqtt_settings):
    ...
    while True:
        ...
        if not deviceInfos.get(emdata["serial"]):
            deviceInfos[emdata["serial"]] = DeviceInfo(
                name="SMA Energy Meter / Sunny Home Manager (2)",
...
for dev in cfg["Devices"]:
    if dev["Type"].lower() == "tripowerx":
        ...
        powermeter = threading.Thread(
            target=updatePowerMeter, args=[dev, mqtt_settings]
        )
